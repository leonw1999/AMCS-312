...
# Ex5 Part 1
echo -e "\n\n############# Ex5 Part 1 ###############\n\n" | tee -a "$LOGFILE"
srun -n 192 --hint=nomultithread ps2-2D -da_grid_x 1024 -da_grid_y 1024 -ksp_view 2>&1 | tee -a "$LOGFILE"
...
for RESTART in 30 15 60 200; do
    ...
    srun -n 192 --hint=nomultithread ps2-2D \
...
echo -e "\n\n############# CG without PC ###############\n\n" | tee -a "$LOGFILE"
srun -n 192 --hint=nomultithread ps2-2D \
    -da_grid_x 1024 -da-grid_y 1024 -ksp_type cg -pc_type none \
    -ksp_rtol 1e-5 -ksp_converged_reason -log_view -ksp_monitor 2>&1 | tee -a "$LOGFILE"

# CG with PC
echo -e "\n\n############# CG with Jacobi ###############\n\n" | tee -a "$LOGFILE"
srun -n 192 --hint=nomultithread ps2-2D \
    -da_grid_x 1024 -da-grid_y 1024 -ksp_type cg -pc_type bjacobi \
    -ksp_rtol 1e-5 -ksp_converged_reason -log_view -ksp_monitor 2>&1 | tee -a "$LOGFILE"


# Direct Solvers with LU and Cholesky
echo -e "\n\n############# LU as direct solver/PC ###############\n\n" | tee -a "$LOGFILE"
srun -n 1 --hint=nomultithread ps2-2D \
    -da_grid_x 1024 -da_grid_y 1024 \
...
for REFINEMENT in 0 2 4 6; do
    ...
    srun -n $((8*(2**$REFINEMENT))) --hint=nomultithread ps2-2D \
...
srun -n 192 --hint=nomultithread ps2-2D \
    ...
    -mg_levels_0_ksp_type chebyshev -mg_levels_0_pc_type jacobi \
    -mg_levels_1_ksp_type gmres -mg_levels_1_pc_type asm \
...
for REFINEMENT in 3 4 5 6; do
    ...
    srun -n $((16*2**$REFINEMENT)) --hint=nomultithread ps2-3D \
    -da_grid_x 16 -da_grid_y 16 -da_grid_z 16 -da_refine $REFINEMENT -ksp_type cg -pc_type bjacobi \
...
for REFINEMENT in 3 4 5 6; do
    ...
    srun -n $((16*2**$REFINEMENT)) --hint=nomultithread ps2-3D \
    -da_grid_x 16 -da_grid_y 16 -da_grid_z 16 -da_refine $REFINEMENT -ksp_type cg -pc_type hypre \
