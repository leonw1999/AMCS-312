...
int main(int argc,char **args) {
    ...
    if (writeMatrix) {
        PetscCall(PetscViewerASCIIOpen(PETSC_COMM_WORLD, "matrix_3D.mtx", &viewer));
    ...
    PetscCall(DMDAGetLocalInfo(da,&info));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                "on %d x %d x %d point grid:  error |u-uexact|_1 = %g\n",
                info.mx,info.my,info.mz,errnorm));

    PetscCall(VecNorm(u,NORM_2,&errnorm));
    PetscCall(DMDAGetLocalInfo(da,&info));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                "on %d x %d x %d point grid:  error |u-uexact|_2 = %g\n",
                info.mx,info.my, info.mz, errnorm));
...
PetscErrorCode formExact(DM da, Vec uexact) {
    ...
    for (k = info.zs; k < info.zs + info.zm; k++) {  
        ...
        for (j = info.ys; j < info.ys + info.ym; j++) {  
            ...
            for (i = info.xs; i < info.xs + info.xm; i++) {  
                x = i * hx;
                auexact[k][j][i] = x*x * (x*x - 1) * y*y * (y*y - 1.0) * z*z * (z*z - 1.0);
            }
        }
    }
        ...
        for (j=info.ys; j<info.ys+info.ym; j++) {
            ...
            for (i=info.xs; i<info.xs+info.xm; i++) {
                ...
                if (i==0 || i==info.mx-1 || j==0 || j==info.my-1 || k==0 || k==info.mz-1) {
                    ...
                    f = -2.0 * x * x * y * y * (x * x - 1.0) * (y * y - 1.0) * (6.0 * z * z - 1.0)  
                    - 2.0 * x * x * z * z * (x * x - 1.0) * (6.0 * y * y - 1.0) * (z * z - 1.0)  
                    - 2.0 * y * y * z * z * (6.0 * x * x - 1.0) * (y * y - 1.0) * (z * z - 1.0);
                    ab[k][j][i] = hx * hy * hz * f;
                }
            }
        }
