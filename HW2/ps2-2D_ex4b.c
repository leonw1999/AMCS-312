...
int main(int argc,char **args) {
    ...
    if (writeMatrix) {
        PetscCall(PetscViewerASCIIOpen(PETSC_COMM_WORLD, "matrix_2D.mtx", &viewer));
    ...
    PetscCall(DMDAGetLocalInfo(da,&info));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                "on %d x %d point grid:  error |u-uexact|_1 = %g\n",
                info.mx,info.my,errnorm));

    PetscCall(VecNorm(u,NORM_2,&errnorm));
    PetscCall(DMDAGetLocalInfo(da,&info));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                "on %d x %d point grid:  error |u-uexact|_2 = %g\n",
                info.mx,info.my,errnorm));
...
PetscErrorCode formRHS(DM da, Vec b) {
    ...
    for (j=info.ys; j<info.ys+info.ym; j++) {
        ...
        for (i=info.xs; i<info.xs+info.xm; i++) {
            x = i * hx;
            if (i==0 || i==info.mx-1 || j==0 || j==info.my-1) {
                ab[j][i] = 3*x + sin(20*x*y);  // on boundary: 1*u = 0
            } else {
                f = 400*(x*x + y*y)*sin(20*x*y);
                ab[j][i] = hx * hy * f;
            }
        }
    }
